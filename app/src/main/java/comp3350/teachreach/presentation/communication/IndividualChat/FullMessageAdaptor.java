...
public class FullMessageAdaptor extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public FullMessageAdaptor(Context context, Map<Integer, Map<Timestamp, String>>  messages){
    ...
    private void extractMessages(){
        ...
        for(Map.Entry<Integer,Map<Timestamp,String>> entry : messages.entrySet()){
            ...
            for(Map.Entry<Timestamp, String> messageEntry : messageData.entrySet()) {
                //int receivedID = (senderID == currentUserID) ? getOtherUserID(senderID): currentUserID;
                if(senderID == currentUserID){
                    sentMessage.put(messageEntry.getKey(),messageEntry.getValue());
                }
                else{
                    receivedMessage.put(messageEntry.getKey(),messageEntry.getValue());
                }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            case VIEW_TYPE_RECEIVED_MESSAGE:
                view = inflater.inflate(R.layout.fragment_other_guy_message, parent, false);
                return new ReceivedMessageViewHolder(FragmentOtherGuyMessageBinding.bind(view));
            default:
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        switch (holder.getItemViewType()) {
            case VIEW_TYPE_SENT_MESSAGE:
                ((SendMessageViewHolder) holder).setMessageData(sentMessage, position);
                break;
            case VIEW_TYPE_RECEIVED_MESSAGE:
                ((ReceivedMessageViewHolder) holder).setMessageData(receivedMessage, position);
                break;
        }
    }
    ...
    class ReceivedMessageViewHolder extends RecyclerView.ViewHolder {
        ...
        void setMessageData(Map<Timestamp, String> receivedMessage, int position){
            // Retrieve message data at the given position
            Timestamp time = (Timestamp) receivedMessage.keySet().toArray()[position];
    ...
    class SendMessageViewHolder extends RecyclerView.ViewHolder{
        ...
        void setMessageData(Map<Timestamp, String> sentMessage, int position){
            // Retrieve message data at the given position
            Timestamp time = (Timestamp) sentMessage.keySet().toArray()[position];
            String message = sentMessage.get(time);

            // Bind message data to views
            binding.myText.setText(message);
            int date = (int) messageHandler.timeStampConverter(time).get("Date");
