...
public
class TutorFilter implements ITutorFilter
{
    ...
    TutorFilter()
    {
        userProfileHandler  = new UserProfileFetcher<>();
        tutorProfileHandler = new TutorProfileHandler();
    }
    ...
    {
        cond.put(ConditionFilterTag.minReview, t -> tutorProfileHandler.getAvgReview(t) >= minimumAvgRating);
        return this;
    }

    @Override
    public
    TutorFilter setMinimumHourlyRate(double desiredHourlyRate)
    {
    ...
    @Override
    public
    TutorFilter setMaximumHourlyRate(double desiredHourlyRate)
    {
    ...
    @Override
    public
    TutorFilter setCourseCode(String courseCode)
    {
        cond.put(ConditionFilterTag.courseCode, t -> tutorProfileHandler.getCourseCodeList(t).contains(courseCode));
        return this;
    }

    @Override
    public
    TutorFilter setSearchFilter(String searchString)
    {
        final String s = searchString.trim().toLowerCase();
        searchCondition = t ->
                tutorProfileHandler.getCourseCodeList(t).stream().anyMatch(code -> code.toLowerCase().contains(s))

                || tutorProfileHandler
                        .getCourseDescriptionList(t)
                        .stream()
                        .anyMatch(desc -> desc.toLowerCase().contains(s))

                || tutorProfileHandler.getPreferredLocations(t).stream().anyMatch(l -> l.toLowerCase().contains(s))
    ...
    @Override
    public
    TutorFilter clearSortByReviews()
    {
    ...
    private
    Predicate<ITutor> condition()
    {
        Predicate<ITutor> filterConditions = cond.values().stream().reduce(Predicate::and).orElse(t -> true);
        return filterConditions.and(searchCondition);
    }

    private
    Comparator<ITutor> sortBy()
    {
        Comparator<ITutor> result = null;

        Comparator<ITutor> byPrice = Comparator.comparingDouble(ITutor::getHourlyRate);

        Comparator<ITutor> byReviews = Comparator.comparingDouble(t -> tutorProfileHandler.getAvgReview(t));
        ...
        for (SortConditionTag t : sortCondSet) {
            switch (t) {
                ...
                case byReviews -> result = (result == null) ? byReviews : result.thenComparing(byReviews);
            }
        }
    ...
    private
    enum ConditionFilterTag
    {
        minReview, minPrice, maxPrice, courseCode
    }

    private
    enum SortConditionTag
    {
        byPrice, byReviews
    }
}
